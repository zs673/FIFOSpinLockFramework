		...
		unsigned int res_id)
{
	...
	//std::cout << " t1 requests: " << num_requests << std::endl;
	...
	//std::cout << " t1 requests with higher priority: " << num_requests << std::endl;
	return num_requests;
}
	...
	LinearProgram& lp)
{
	...
	foreach_task_except(info.get_tasks(), ti, tx)
	{
		...
		foreach(tx->get_requests(), request)
		{
			...
			//std::cout << "request: "<< q << std::endl;

			// unsigned long ri = ti.get_response();
			// unsigned int __max_num_requests = request->get_max_num_requests(ri);
			//std::cout << "ri: "<< ri << "  tx requests: " << __max_num_requests << std::endl;
				 
			foreach_request_instance(*request, ti, v)
			{

				//std::cout << "request number: "<< v << std::endl;
				LinearExpression *exp = new LinearExpression();
				unsigned int var_id, var_id2;

				var_id = vars.lookup(t, q, v, BLOCKING_DIRECT);
				exp->add_var(var_id);

				var_id2 = vars.lookup(t, q, v, BLOCKING_ARRIVAL);
				exp->add_var(var_id2);
//				std::cout << t << " " << q << " " << v << " " << var_id << std::endl;
