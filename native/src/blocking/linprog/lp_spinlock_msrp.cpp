...
//	 if(i==0){
//	 	 for (unsigned int i = 0; i < vars.get_num_vars(); i++)
//	 	{
//	 		std::cout << vars.var2str(i) << std::endl;
//	 	}
//	 }
//
//
//std::cout << " number of variables" << vars.get_num_vars() << std::endl;
    ...
    bounds[i] = total;
    bounds.set_remote_blocking(i, lrint(sol->evaluateSpin(*lp.get_objective())));
    bounds.set_arrival_blocking(i, lrint(sol->evaluateArrival(*lp.get_objective())));
