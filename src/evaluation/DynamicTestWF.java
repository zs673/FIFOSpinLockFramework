		...
		for (int i = 0; i < TOTAL_NUMBER_OF_SYSTEMS; i++) {
			Thread worker = new Thread(new Runnable() {
				...
				public void run() {
					...
					Ris = IOAmrsp.getResponseTime(tasks, resources, true, false, AnalysisUtils.extendCalForStatic, useRi);
					if (isSystemSchedulable(tasks, Ris))
						inciamrsp();

					Ris = IOAfnp.getResponseTime(tasks, resources, true, false, AnalysisUtils.extendCalForStatic, useRi);
					if (isSystemSchedulable(tasks, Ris))
						inciafnp();

					Ris = IOAfp.getResponseTime(tasks, resources, true, false, AnalysisUtils.extendCalForStatic, useRi);
					if (isSystemSchedulable(tasks, Ris))
						inciafp();
	...
	public void parallelExperimentIncreasingCriticalSectionLength(int cslen) {
		...
		for (int i = 0; i < TOTAL_NUMBER_OF_SYSTEMS; i++) {
			Thread worker = new Thread(new Runnable() {
				...
				public void run() {
					SystemGenerator generator = new SystemGenerator(MIN_PERIOD, MAX_PERIOD, TOTAL_PARTITIONS,
							TOTAL_PARTITIONS * NUMBER_OF_TASKS_ON_EACH_PARTITION, true, cs_range, RESOURCES_RANGE.PARTITIONS, RSF,
