...
public class CompleteConfidence {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		CompleteConfidence test = new CompleteConfidence();
	...
	public void parallelExperimentIncreasingParallelism(int NoP, Counter counter) {
				...
				public void run() {
					...
					for (int i = 0; i < TOTAL_NUMBER_OF_SYSTEMS; i++) {
						...
						Thread worker = new Thread(new Runnable() {
							...
							@Override
							public void run() {
								SystemGenerator generator = new SystemGenerator(MIN_PERIOD, MAX_PERIOD, true, NoP, NoP * NUMBER_OF_TASKS_ON_EACH_PARTITION, RSF,
						...
						worker.setName("4 " + NoP + " times: " + fatherindex + " numbers: " + childindex);
						worker.start();
					}
					...
					String subresult = NoP + "|" + fatherindex + " " + subcount.fnp + " " + subcount.fp + " " + subcount.mrsp + " " + subcount.Dcombine + " "
							+ subcount.Dnew;
					writeSystem("4 2 " + NoP + " " + fatherindex, subresult);
		...
		String result = "access-" + NoP + " " + counter.fnp + " " + counter.fp + " " + counter.mrsp + " " + counter.Dcombine + " " + counter.Dnew;
