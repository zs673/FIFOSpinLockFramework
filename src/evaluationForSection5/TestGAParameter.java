		...
		for (int i = 0; i < TOTAL_NUMBER_OF_SYSTEMS; i++) {
			Thread worker = new Thread(new Runnable() {
				...
				public void run() {
					...
					GASolver solver = new GASolver(tasksToAlloc, resources, generator, ALLOCATION_POLICY, PRIORITY_RULE, GENERATIONS, POPULATION, 5, 0.4, 0.1,
							5, 5, 5, true);

					solver.population = 100;
					solver.checkSchedulability(true);
					
					solver.population = 200;
					solver.checkSchedulability(true);
					
					solver.population = 400;
					solver.checkSchedulability(true);
					
					solver.population = 600;
					solver.checkSchedulability(true);
					
					solver.population = 800;
					solver.checkSchedulability(true);
					
					solver.population = 1000;
					solver.checkSchedulability(true);
